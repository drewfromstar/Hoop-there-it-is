...
function CreateGameView({ setView, games, saveGames, currentUser, allPlayers, setAllPlayers, showAddPlayer, setShowAddPlayer, newPlayerName, setNewPlayerName, addPlayer }) {
  ...
  // Update available players when allPlayers changes (e.g., when a new player is added)
  useEffect(() => {
    const alreadyInPriority = priorityList.map(p => p.id);
    setAvailablePlayers(allPlayers.filter(p => !alreadyInPriority.includes(p.id)));
  }, [allPlayers, priorityList]);
  ...
  const deletePlayer = async (playerId, e) => {
    e.stopPropagation(); // Prevent adding player when clicking delete
    if (confirm('Delete this player permanently?')) {
      const updatedPlayers = allPlayers.filter(p => p.id !== playerId);
      setAllPlayers(updatedPlayers); // Update parent state
      await window.storage.set('all-players', JSON.stringify(updatedPlayers));
      setAvailablePlayers(availablePlayers.filter(p => p.id !== playerId));
      setPriorityList(priorityList.filter(p => p.id !== playerId));
    }
  };
            ...
            {availablePlayers.map(player => (
              ...
              >
                <button
                  ...
                  className="flex-1 text-left hover:text-orange-600 transition-colors"
                >
                  {player.name}
                </button>
                <button
